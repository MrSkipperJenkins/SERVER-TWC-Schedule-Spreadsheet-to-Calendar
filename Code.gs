...
// web app url - https://script.google.com/a/macros/weathergroup.com/s/AKfycbybOOIk5W_1Q-cThiJbTx_cXBgORZGTaQAoDRZ28gWCiVJr6BOi/exec
// web app dev - https://script.google.com/a/macros/weathergroup.com/s/AKfycbw2DsNzGVSin3Kgtsf8oa1KB7lk7eW_rMfsBDa8nif8/dev

function myTeamServerFunction() {
  var scheduleSheetId = PropertiesService.getScriptProperties().getProperty('scheduleEventList'); //id of "Schedule Event List for each user"
  ...
  writeAllToNamedDB(parseSchedule(sheets.thisWeek).concat(parseSchedule(sheets.nextWeek), 
                                                          parseSchedule(sheets.thirdWeek), 
                                                          parseSchedule(sheets.fourthWeek)), ss);
          ...
          for (var i = 5; i < lastRow; i++) { //get all jobs for each worker
            ...
            if ((valueAt[i][Worker].trim()) && (valueAt[i][Worker].search('PTO') == -1) && (valueAt[i][Worker].search('OPH') == -1)) {  
              ...
              for (var u = i; u < lastRow; u++) { //find show names       
                ...
                if (((Job==NextJob) && (Show==NextShow)) || (((Job ==  'PRE-PRO') || (Job ==  'PREPRO') || (Job ==  'PROJECT')) && (Job==NextJob))) {
                  ECount += 1;}
                else {
                  var CurrentRow = u;
                  u = lastRow;
              ...
              if ((Job == 'BWTM') || (Job ==  'PRE-PRO') || (Job ==  'PREPRO') || (Job ==  'PROJECT')) {
...
//  }
  ...
  var LastMonth = (Today.getMonth() == 0) ? 'DECEMBER' : MonthOfYear[Today.getMonth()-1];
  var NextMonth = MonthOfYear[Today.getMonth()+1] || "JANUARY";
  ...
  while (folderSheets.hasNext()) {
    ...
    var WeekOfNum = findWeekDigits(WeekOf.slice(W+Month.length)); //we need to extract the number value from this string using a regex
    ...
    if ((Math.abs(Day-WeekOfNum) > 14) && (Math.abs(Day-WeekOfNum) <= 21) && ((Day-WeekOfNum) < 0)) {
      var FourthWeekSheet = Sheets; 
      Logger.log('Fourth weeks calender: ' + FourthWeekSheet);
    }
  }  
  ...
  //find next, third, and fourth week's sheet if we need to go into next month 
  if (!NextWeekSheet) { 
    ...
    while (folderSheets.hasNext()) {
      ...
      if ((WeekOfNum > 14) && (WeekOfNum <= 21)) {
        var FourthWeekSheet = Sheets;    
        Logger.log('This is the third week of next month for fourth sheet: ' + FourthWeekSheet);
      }
    }      
  }  
  
  //find third & fourth week's sheet if we need to go into next month 
  if (!ThirdWeekSheet) {
    ...
    while (folderSheets.hasNext()) {
      ...
      if ((WeekOfNum > 7) && (WeekOfNum <= 14)) {
        var FourthWeekSheet = Sheets;    
        Logger.log('This is the second week of next month for fourth sheet: ' + FourthWeekSheet);
      }
    }      
  }
  
  //find fourth week's sheet if we need to go into next month 
  if (!FourthWeekSheet) {
    Logger.log('Fourth week not found, searching next months folder..');
    var DirFolder = Dfolder.searchFolders('title contains "'+ NextMonth +'"');
    while (DirFolder.hasNext()) {
      var folder = DirFolder.next();
      Logger.log(folder.getName());
    ...
    while (folderSheets.hasNext()) {
      ...
      if (WeekOfNum <= 7) {
        var FourthWeekSheet = Sheets;    
        Logger.log('This is the first week of next month for the fourth sheet: ' + FourthWeekSheet);
      }
    }      
  }
  ...
  return {thisWeek: ThisWeekSheet, nextWeek: NextWeekSheet, thirdWeek: ThirdWeekSheet, fourthWeek: FourthWeekSheet, calendarStart: calendarStart(ThisWeekSheet), calendarEnd: calendarEnd(ThirdWeekSheet)};
}

function findWeekDigits(str) {
   //we need to extract the number value from this string using a regex
    var patt1 = /\d/g;
...
function getShiftTime(val) {
  //(COMMENT THIS LINE OUT) var val = '630A - 230P'; //sample value for debugging (COMMENT THIS LINE OUT)
  var shift = val.trim().toUpperCase().split('-'); //remove possible outside whitespace / split into two different strings at the "-"
  
  for (var a = 0; a < shift.length; a++) { //remove possible inside whitespace
    shift[a] = shift[a].trim();
  }
...
function getEmailFromScheduleName(name) {
  switch (name) {
    ...
    case "Caleb Bowen":
      return "caleb.bowen@weathergroup.com";
      break;
